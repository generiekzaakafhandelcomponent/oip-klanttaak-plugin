...
export class DelegateTaskComponent implements FunctionConfigurationComponent, OnInit, OnDestroy {
  ...
  readonly levelOfAssuranceOptions: Array<SelectItem> = Object.keys(LevelOfAssurance).map(key => (
    {
      id: key,
      text: LevelOfAssurance[key as keyof typeof LevelOfAssurance]
    }
  ));
  readonly koppelingRegistratieOptions: Array<SelectItem> = Object.keys(Registratie).map(key => (
    {
      id: key,
      text: Registratie[key as keyof typeof Registratie]
    }
  ));
